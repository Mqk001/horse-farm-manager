...
import { CalendarDays, ChevronRight, Home, Menu, Plus, Settings, Sparkles, X } from 'lucide-react';
...
function SidebarContents({ pathname, close }: { pathname: string; close: () => void }) {
  ...
  return (
    <>
      ...
      <div className="m-4 rounded-2xl border border-white/[.08] bg-white/[.04] p-4"><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-full bg-[#d6b77b]/15 text-[#d6b77b]"><Settings className="h-4 w-4" /></div><div><p className="text-xs font-semibold text-white/85">Stable workspace</p><p className="mt-0.5 text-[11px] text-white/40">All systems ready</p></div></div></div>
    </>
  );
}
...
export function Navbar() {
  ...
  return (
    <>
      ...
      <aside className="fixed inset-y-0 left-0 z-50 hidden w-[17.5rem] flex-col overflow-hidden bg-[#112b21] lg:flex"><SidebarContents pathname={pathname} close={() => setOpen(false)} /></aside>
      {open && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <aside className="relative flex h-full w-[18rem] flex-col bg-[#112b21] shadow-2xl">
            ...
            <SidebarContents pathname={pathname} close={() => setOpen(false)} />
          </aside>
        </div>
      )}
    </>
